...
from portal.paths import BROWSER_PATH, ICON_PATH, LOGO_PNG_PATH
...
class CrawlerLauncher:
    ...
    def _toast(self, title: str, msg: str):
        try:
            icon = str(LOGO_PNG_PATH) if LOGO_PNG_PATH.exists() else ""
    ...
    def _base_url(self) -> str:
        ...
        return f"http://{display_host}:{config['api']['port']}"
    ...
    def _setup_tray(self):
        ...
        menu = pystray.Menu(
            pystray.MenuItem("Show GovCrawler", self._tray_show, default=True),
            pystray.MenuItem("Open Web Interface", self._tray_open_browser,
                             enabled=lambda item: self.state == AppState.RUNNING),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Quit", self._tray_quit),
        )
        self.tray_icon = pystray.Icon("govcrawler", image, "GovCrawler Control Panel", menu)
    ...
    def _load_tray_image() -> Image.Image:
        try:
            if LOGO_PNG_PATH.exists():
                return Image.open(str(LOGO_PNG_PATH))
        except Exception as e:
            log.warning(f"Could not load tray icon from {LOGO_PNG_PATH}: {e}")
    ...
    def _tray_quit(self, icon=None, item=None):
        self.root.after(0, self._request_quit, True)

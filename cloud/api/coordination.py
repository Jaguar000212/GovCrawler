...
async def coordination_create_job(
    ...
    db.start_job(job_id)
    db.write_audit(user.id, "job.create", "job", job_id, detail={"seed_count": len(seeds)}, ip=client_ip(request))
...
async def coordination_finish_job(
    ...
    db.write_audit(
        ...
        "job",
        job_id,
        detail={"status": payload.status, "error": payload.error},
...
async def coordination_resume_job(
    ...
    if req.agent_id and not db.claim_or_verify_job_agent(job_id, req.agent_id):
        ...
        raise HTTPException(
            status_code=403,
            detail="This job was started by a different agent and can only be resumed from that machine",
        )
